...
/*int spf_delta_set1(spfbuf_t *obuf)
{
	unsigned long i;
	unsigned short j;
	...
	for (i = 0; i < obuf->n; i++) {
		...
		next1 = next2;
		if (i > 1)
			prev1 = prev2 + obuf->adim;
		if (i + 2 < obuf->n)
			next2 = next1 + obuf->adim;
		
		for (j = 0; j < obuf->dim; j++)
...
{
  if ((p->s = (spf_t*)realloc(p->s, n * p->adim * sizeof(spf_t))) != NULL)
    p->m = n;
...
spfstream_t *spf_input_stream_open(const char *fn, long flag, size_t nbytes)
{
  ...
  sp_swap1(&(s->iflag), SIZEOF_LONG);
  sp_swap2(&(s->Fs), sizeof(float));
#endif
  ...
  if (s->oflag != s->iflag) {
    ...
    s->odim = spf_tot_dim((unsigned short)(idx[1] + 1),(long)(s->oflag));
  }
  else
    s->odim = s->idim;
				    ...
				    long cflag, float frate, const spfield_t *vh, size_t nbytes)
{
  ...
  sp_swap1(&flag, SIZEOF_LONG);
  sp_swap2(&rate, sizeof(float));
#endif
