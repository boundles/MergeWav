 ...
 * different modes:
 */
spfbuf_t *spf_buf_convert(spfbuf_t *ibuf, int iflag, int oflag, unsigned long wl, int mode)
{
  unsigned long i;
  ...
  if (((oflag & WITHZ) && ! (iflag & WITHZ)) || ((oflag & WITHR) && ! (iflag & WITHR)))
    if (spf_buf_normalize(obuf, 0, (unsigned short)(sdim - 1), (unsigned short)wl, oflag & WITHR) != 0) {
  ...
  if (oflag & WITHA)
    spf_delta_set(obuf, obins[3], (unsigned short)((oflag & WITHE) ? (sdim + 1): (sdim)), obuf, obins[6]);
...
void spf_delta_set(spfbuf_t *ibuf, unsigned short ik, unsigned short dim, spfbuf_t *obuf, unsigned short ok)
{
  ...
  for (i = 0; i < ibuf->n; i++) {
    ...
    for (j = 0; j < dim; j++)
      *(op+j) =(float)((*(next1+j) - *(prev1+j) + 2.0 * (*(next2+j) - *(prev2+j))) * 0.1);
